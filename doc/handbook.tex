...
\begin{figure}[hb]
  ...
  \caption{Resultat der Beispieldatei}
\end{figure}
...
\chapter{Briefe im Stil der TU Dresden}

Briefe für die Hauspost, sowie interne und externe Kommunikation werden mit der Klasse \lstinline|tudcdlttr|
umgesetzt. Hierbei wurde die Klasse \texttt{scrlttr}~\cite{ctan-scrlttr2} erweitert und mit den entsprechenden
Einstellungen konfiguriert.

Um ein einheitliches Interface der TUDCD-Klassen zu gewährleisten, wird zum Zeitpunkt des Schreibens
\emph{keine} \texttt{.lco} Datei im Sinne von \cite{ctan-scrlttr2} bereitgestellt, sondern lediglich eine
Klassendatei.

\section{Erweiterungen von \texttt{ctan-scrlttr2}}

Die Einstellungen für zusätzliche Merkmale des Briefes sind mittels der
folgend angegebenen \KOMAScript-Variablen konfigurierbar.

\begin{table}[H]
  \centering
  \begin{tabular}{rl}
    \toprule
 \KOMAScript-Variable & Beschreibung \\
    \midrule
 \texttt{tudprincipalline} & Schriftzug für \emph{Die Rektorin} \\
 \texttt{tudpostaladdress} & Postaddresse der TU Dresden \\
 \texttt{tudtaxid} & Steueridentifikationsnummer \\
 \texttt{tudturnovertaxid} & Umsatzsteuernummer \\
 \texttt{backaddress} & Rücksendeadresse \\
 \texttt{toname} & Addressat \\
 \texttt{toaddress} & Adresse des Addressaten \\
 \texttt{fromentity} & Struktureinheit \\
 \texttt{fromlogo} & Zweitlogo der Struktureinheit \\
 \texttt{fromname} & Name der Person \\
 \texttt{fromposition} & Position der Person \\
 \texttt{fromphone} & Telefonnummer \\
 \texttt{fromfax} & Faxnummer \\
 \texttt{fromemail} & Email-Addressen \\
 \texttt{fromurl} & Website \\
 \texttt{fromaddress} & Besuchsaddresse \\
 \texttt{frombank} & Bankverbindung \\
 \texttt{frombic} & Bank Identifier Code \\
 \texttt{yourref} & Aktenzeichen \\
 \texttt{date} & Datum \\
    \bottomrule
  \end{tabular}
\end{table}

Alle angegebenen Informationen können mittels \lstinline|\setkomavar| auf entsprechende Werte gesetzt werden.

Fast alle Informationen werden in der \emph{Absenderergänzung} gesetzt und werden mittels der \KOMAScript-Variablen
\lstinline|toplocation| und \lstinline|bottomlocation| gesetzt. Sollte die Anordnung nicht dem Zweck entsprechen, dann können
diese Informationen angepasst werden.

\section{Setzen von Zweitlogos}

Zweitlogos haben die Vorgabe maximal $2/3$ des TUD Logos einzunehmen. Um diese Größenskalierung zu automatisieren,
wird der Befehle \lstinline|TypesetSecondLogo| bereitgestellt, welcher den Inhalt automatisch auf die zulässige Größe verkleinert.
Um beispielsweise ein Zweitlogo zentriert unter der Struktureinheit platzieren zu können, kann
\begin{lstlisting}
\setkomavar{fromlogo}{\centering \TypesetSecondLogo{\includegraphics{example-image-a}} }
\end{lstlisting}
genutzt werden.

\section{Fertiger Beispielbrief}

Ein fertiges Beispiel für einen Brief wird bereitgestellt.

\lstinputlisting[linerange=\%\ Start-\%\ End]{demo-pres.tex}
\marginpar{Klicken Sie hier um die Datei aus der PDF zu extrahieren.
\textattachfile{demo-pres.tex}{\texttt{demo-pres.tex}}}
...
Zum Beispiel ist das Einstellen der Optionen herkömmlich über \emph{gewöhnliche Klassenoptionen}
gelöst, und nicht über \lstinline|\TUDCDsetup|. An einer Aktualisierung wird gearbeitet.

Dies liegt unter anderem daran, dass diese Klasse parallel zu den anderen Klassen entstanden ist,
und eine Überarbeitung noch geplant ist.
...
Die Titelfolie, welche mit \lstinline|\maketitle| gesetzt wird, kann mittels der verschiedenen Einstellungen über
die allgemeine Farbauswahl beeinflusst werden.
\begin{lstlisting}
\TUDCDsetup{
  color = {
    highlight-color=Weiss,
    secondary-color=Brilliantblau
  },
  logo = {
    color = white
  }
...
Die Fußzeilen der Folien können über die Schlüssel \lstinline|beamer/footline| bzw.\@ \lstinline|beamer/title-footline|
konfiguriert werden. Es wird dabei eine kommaseparierte Liste von Feldern erwartet, welche durch ein \lstinline|\textbullet|
voneinander getrennt werden.
\begin{lstlisting}
\TUDCDsetup{
  beamer = {
    title-footline = {\insertdate,\insertlocation},
    footline = {\insertshorttitle,\insertshortauthor,\pagename \ \insertframenumber}
  }
}
\end{lstlisting}

Leerzeichen müssen manuell mit \lstinline[showspaces=true]|\ | eingefügt werden.
...
\begin{figure}[hb]
  ...
  \setlength\fboxsep{0pt}
  \subcaptionbox{Erste Seite}{
    \fbox{\includegraphics[page=1,width=\dimexpr(2\tudcdGridColumnwidth+1\tudcdGridColumnsep)]{demo-pres.pdf}}
  }
  \subcaptionbox{Zweite Seite}{
    \fbox{\includegraphics[page=2,width=\dimexpr(2\tudcdGridColumnwidth+1\tudcdGridColumnsep)]{demo-pres.pdf}}
  }

  \subcaptionbox{Dritte Seite}{
    \fbox{\includegraphics[page=3,width=\dimexpr(2\tudcdGridColumnwidth+1\tudcdGridColumnsep)]{demo-pres.pdf}}
  }
  \subcaptionbox{Vierte Seite}{
    \fbox{\includegraphics[page=4,width=\dimexpr(2\tudcdGridColumnwidth+1\tudcdGridColumnsep)]{demo-pres.pdf}}
  }
  \caption{Resultat der Beispieldatei}
\end{figure}
